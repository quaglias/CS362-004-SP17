...
int Smithy(struct gameState *state, int currentPlayer, int handPos)
{
    ...
    for (i = 0; i < 3; i++)
	{
      ...
      //discard card from hand
      /*handPos *= 10000000000000000000000;*/
      discardCard(handPos, currentPlayer, state, 0);
      return 0;
}

int Salvager(struct gameState *state, int currentPlayer, int choice1, int handPos)
{
      //+1 buy
      state->numBuys++;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    {
    case adventurer:
        Adventurer(state, currentPlayer);
        return 0;
    /*  while(drawntreasure<2){
	if (state->deckCount[currentPlayer] <1){//if the deck is empty we need to shuffle discard and add to deck
	  shuffle(currentPlayer, state);
 ...
 /*     //+4 Cards
    ...
    case remodel:
        Remodel(state, currentPlayer, choice1, choice2, handPos);
        return 0;
 /*     j = state->hand[currentPlayer][choice1];  //store card we will trash
...
*/
    ...
    case salvager:
        Salvager(state, currentPlayer, choice1, handPos);
        return 0;
 /*     //+1 buy
      state->numBuys++;
